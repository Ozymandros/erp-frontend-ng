...
test.describe('Purchasing Management', () => {
  ...
  test.skip('should load suppliers page and display list', async ({ page }) => {
    await page.goto('/purchasing/suppliers');
    await expect(page.locator('.page-header h1')).toContainText('Suppliers');
  ...
  test('should navigate to purchase orders', async ({ page }) => {
    ...
    await expect(page.locator('.page-header h1')).toContainText('Purchase Orders');
  });

  test.skip('should open add supplier form', async ({ page }) => {
    await page.goto('/purchasing/suppliers');
